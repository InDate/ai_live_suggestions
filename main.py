...
def create_ui_components(root):
    ...
    transcript_textbox = ctk.CTkTextbox(
        root, width=300, font=("Arial", font_size), text_color="#FFFCF2", wrap="word"
    )
    transcript_textbox.grid(row=0, column=0, padx=10, pady=20, sticky="nsew")

    response_textbox = ctk.CTkTextbox(
        root, width=300, font=("Arial", font_size), text_color="#639cdc", wrap="word"
    )
...
def main():
    ...
    global_transcriber = AudioTranscriber(
        user_audio_recorder.source, speaker_audio_recorder.source
    )
    transcribe = threading.Thread(
        target=global_transcriber.transcribe_audio_queue, args=(audio_queue,)
    )
    ...
    respond = threading.Thread(
        target=responder.respond_to_transcriber, args=(global_transcriber,)
    )
    respond.daemon = True
    respond.start()
